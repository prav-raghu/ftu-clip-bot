...
var Security = class {
  ...
  static isAllowedURL(url2, config2) {
    return [config2.openrouter.baseURL, config2.local.baseURL].some(
      (base) => url2.startsWith(base)
    );
  }
};
...
function createWindow() {
  ...
  win.loadURL(
    process.env.VITE_DEV_SERVER_URL || `file://${import_node_path2.default.join(process.cwd(), "dist/index.html")}`
  );
  win.webContents.on("will-navigate", (e) => e.preventDefault());
  win.webContents.setWindowOpenHandler(() => ({ action: "deny" }));
}
function createTray() {
  const iconPath = import_node_path2.default.join(process.cwd(), "assets", "mascot.png");
  tray = new import_electron2.Tray(iconPath);
  tray.setToolTip("FTU Clip Bot");
  tray.setContextMenu(
    import_electron2.Menu.buildFromTemplate([
      {
        label: "Show/Hide",
        click: () => {
          if (!win) return;
      ...
      },
      {
        ...
        click: () => {
          config.provider = config.provider === "openrouter" ? "local" : "openrouter";
          Storage.saveConfig(config);
          win?.webContents.send("config:update", config);
        }
      },
      { type: "separator" },
      { label: "Quit", click: () => import_electron2.app.quit() }
    ])
  );
}
function setupHotkeys() {
  if (!config.ui.hotkeysEnabled) return;
  import_electron2.globalShortcut.register(
    "Control+Shift+Space",
    () => win?.isVisible() ? win?.hide() : win?.show()
  );
  import_electron2.globalShortcut.register(
...
import_electron2.app.whenReady().then(() => {
  ...
  createTray();
  setupHotkeys();
  import_electron2.app.setLoginItemSettings({ openAtLogin: config.ui.startOnLogin });
  import_electron2.app.on(
    "activate",
    () => import_electron2.BrowserWindow.getAllWindows().length === 0 && createWindow()
  );
});
import_electron2.app.on("window-all-closed", () => {
  if (process.platform !== "darwin") import_electron2.app.quit();
});
import_electron2.app.on("will-quit", () => import_electron2.globalShortcut.unregisterAll());
import_electron2.ipcMain.handle("getConfig", async () => {
  config = Storage.loadConfig();
  return { ...config, apiKeySet: await Storage.apiKeySet() };
});
import_electron2.ipcMain.handle("saveConfig", async (_evt, partial) => {
  config = Storage.saveConfig(partial);
});
import_electron2.ipcMain.handle("setApiKey", async (_evt, key) => {
  await Storage.setApiKey(key);
});
import_electron2.ipcMain.handle("clearApiKey", async () => {
  await Storage.clearApiKey();
});
import_electron2.ipcMain.handle("toggleWindow", () => {
  if (!win) return;
  win.isVisible() ? win.hide() : win.show();
});
import_electron2.ipcMain.handle("focusInput", () => win?.webContents.send("focus:input"));
import_electron2.ipcMain.handle("chat", async (_evt, payload) => {
  const parsed = ChatRequestSchema.safeParse(payload);
  if (!parsed.success) throw new Error("Invalid chat payload");
  const { messages, model, provider } = parsed.data;
  const baseURL = provider === "openrouter" ? config.openrouter.baseURL : config.local.baseURL;
  if (!Security.isAllowedURL(baseURL, config))
    throw new Error("Blocked baseURL");
  const apiKey = provider === "openrouter" ? await Storage.getApiKey() : void 0;
  const headers = {
...
import_electron2.ipcMain.handle("startChatStream", async (_evt, payload) => {
  const parsed = ChatRequestSchema.safeParse(payload);
  if (!parsed.success) throw new Error("Invalid chat payload");
  const { messages, model, provider } = parsed.data;
  const baseURL = provider === "openrouter" ? config.openrouter.baseURL : config.local.baseURL;
...
import_electron2.ipcMain.handle("cancelChatStream", async (_evt, id) => {
